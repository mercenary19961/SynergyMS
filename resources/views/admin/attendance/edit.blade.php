...
<div class="flex h-screen">
    <div class="flex-1 p-6 bg-gray-100">
        ...
        <x-title-with-back title="Edit Attendance Record" />

        <!-- Form -->
        <form method="POST" action="{{ route('admin.attendance.update', $attendance->id) }}" class="space-y-4" x-data="attendanceForm()">
            @csrf
            @method('PUT')
            ...
            <div class="flex flex-col md:flex-row md:space-x-4">
                <!-- Employee Dropdown -->
                <div x-data="{ open: false, selectedEmployeeName: '{{ $attendance->employee->user->name }}', selectedEmployeeId: '{{ $attendance->employee_id }}' }" class="md:w-1/2 relative">
                ...
                <!-- Status Dropdown -->
                <div x-data="{ open: false, selectedStatus: '{{ $attendance->status }}' }" class="md:w-1/2 relative">
            ...
            <div class="flex flex-col md:flex-row md:space-x-4">
                ...
                <div class="md:w-1/2">
                    ...
                    <input type="date" name="attendance_date" id="attendance_date" class="mt-1 block w-full border border-gray-300 focus:border-orange-500 focus:outline-none rounded-md p-2" value="{{ $attendance->attendance_date }}">
                </div>

                <!-- Clock In -->
                <div class="md:w-1/2">
                    <label for="clock_in" class="block text-sm font-medium text-gray-700">Clock In</label>
                    <input type="time" name="clock_in" id="clock_in" class="mt-1 block w-full border border-gray-300 focus:border-orange-500 focus:outline-none rounded-md p-2" value="{{ $attendance->clock_in }}">
                </div>

                <!-- Clock Out -->
                <div class="md:w-1/2">
                    <label for="clock_out" class="block text-sm font-medium text-gray-700">Clock Out</label>
                    <input type="time" name="clock_out" id="clock_out" class="mt-1 block w-full border border-gray-300 focus:border-orange-500 focus:outline-none rounded-md p-2" value="{{ $attendance->clock_out }}">
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row md:space-x-4">
                <div class="md:w-1/2">
                    ...
                    <input type="number" name="total_hours" id="total_hours" class="mt-1 block w-full border border-gray-300 focus:border-orange-500 focus:outline-none rounded-md p-2" value="{{ $attendance->total_hours }}">
                </div>

                <div class="md:w-1/2">
                    <label for="leave_hours" class="block text-sm font-medium text-gray-700">Leave Hours</label>
                    <input type="number" name="leave_hours" id="leave_hours" class="mt-1 block w-full border border-gray-300 focus:border-orange-500 focus:outline-none rounded-md p-2" value="{{ $attendance->leave_hours }}">
                </div>
            </div>

            <x-form.button-submit label="Update Attendance" />
        </form>
    </div>
</div>
@endsection
