        ...
        <form method="POST" action="{{ route('admin.projects.update', $project->id) }}">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="lg:col-span-1">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $project->name) }}" placeholder="Project Name" class="mt-1 block w-full border border-gray-300 focus:border-orange-500 focus:outline-none rounded-md p-1.5" required>
                    @error('name')
                ...
                <div x-data="{ open: false, selectedDepartment: '{{ old('department_id', $project->department->name) }}', selectedDepartmentId: '{{ $project->department_id }}', selectedProjectManagerId: '{{ $project->project_manager_id }}' }" class="lg:col-span-1 relative">
                ...
                <div x-data="{ open: false, selectedClient: '{{ old('client_id', $project->client->company_name) }}', selectedClientId: '{{ $project->client_id }}' }" class="lg:col-span-1 relative">
                ...
                <div x-data="{ open: false, selectedStatus: '{{ old('status', $project->status) }}' }" class="lg:col-span-1 relative">
                ...
                <div class="lg:col-span-1">
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $project->start_date ? $project->start_date->format('Y-m-d') : '') }}" class="mt-1 block w-full border border-gray-300 focus:border-orange-500 focus:outline-none rounded-md p-1.5" required>
                    @error('start_date')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="lg:col-span-1">
                    ...
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $project->end_date ? $project->end_date->format('Y-m-d') : '') }}" class="mt-1 block w-full border border-gray-300 focus:border-orange-500 focus:outline-none rounded-md p-1.5">
                    @error('end_date')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="lg:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4" class="mt-1 block w-full border border-gray-300 focus:border-orange-500 focus:outline-none rounded-md p-1.5">{{ old('description', $project->description) }}</textarea>
                    @error('description')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
                </div>
