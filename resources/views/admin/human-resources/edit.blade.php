...
<div class="flex h-screen">
    <div class="flex-1 p-6 bg-gray-100">
        ...
        <x-title-with-back title="Edit HR Employee" />

        <!-- Include Validation Errors -->
        @include('components.form.errors')

        <!-- Form for Editing HR Employee -->
        <form action="{{ route('admin.human-resources.update', $hrEmployee->id) }}" method="POST" enctype="multipart/form-data" x-data="employeeForm({{ $departments->toJson() }}, {{ $hrEmployee->toJson() }})">
            @csrf
            @method('PUT') <!-- We use PUT for updating resources -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" class="mt-1 block w-full border border-gray-300 rounded-md p-2 focus:border-orange-500 focus:outline-none" value="{{ old('name', $hrEmployee->user->name) }}" placeholder="Enter Employee Name">
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="email" id="email" class="mt-1 block w-full border border-gray-300 rounded-md p-2 focus:border-orange-500 focus:outline-none" value="{{ old('email', $hrEmployee->user->email) }}" placeholder="Enter Email">
                    @error('email')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                @include('components.form.gender', ['selectedGender' => $hrEmployee->user->gender])
                ...
                <div class="mb-2">
                    ...
                    <input type="text" name="contact_number" id="contact_number" placeholder="Starts with country code e.g. +962" value="{{ old('contact_number', $hrEmployee->contact_number) }}" class="mt-1 block w-full border border-gray-300 rounded-md p-2 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 focus:outline-none">
                ...
                <div class="mb-2">
                    ...
                    <input type="email" name="company_email" id="company_email" placeholder="Enter company email" value="{{ old('company_email', $hrEmployee->company_email) }}" class="mt-1 block w-full border border-gray-300 rounded-md p-2 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 focus:outline-none">
                    @error('company_email')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-2">
                    ...
                    @error('image')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    @if($hrEmployee->user->image)
                        <img src="{{ asset('storage/' . $hrEmployee->user->image) }}" alt="Current Profile Image" class="mt-2 w-24 h-24 rounded-full object-cover">
                    @endif
                </div>
                ...
                <div class="relative mb-4">
                    ...
                    <input type="hidden" name="department_id" :value="selectedDepartmentId" value="{{ old('department_id', $hrEmployee->department_id) }}">
                    @error('department_id')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="relative mb-4">
                    ...
                    <input type="hidden" name="position_id" :value="selectedPositionId" value="{{ old('position_id', $hrEmployee->position_id) }}">
                    @error('position_id')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <!-- Submit Button -->
            <x-form.button-submit label="Update HR Employee" />
        </form>
    </div>
</div>

<script>
function employeeForm(departments, hrEmployee) {
    return {
        open: false,
        openPosition: false,
        selectedDepartment: hrEmployee.department.name,
        selectedDepartmentId: hrEmployee.department_id,
        selectedPosition: hrEmployee.position.name,
        selectedPositionId: hrEmployee.position_id,
        positions: departments.find(d => d.id == hrEmployee.department_id).positions.map(p => p.name),
