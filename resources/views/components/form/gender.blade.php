<!-- Gender Dropdown -->
<div x-data="{ open: false, selected: '{{ old('user_gender') ?? 'Select Gender' }}' }" class="relative">
    ...
    <ul x-show="open" @click.away="open = false" class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none">
        <li @click="selected = 'Male'; $refs.user_gender.value = 'male'; open = false" class="group cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-orange-500 hover:text-white">
            <span class="flex items-center">
                <i class="fas fa-male mr-2 text-orange-500 group-hover:text-white"></i> Male
            </span>
        </li>
        <li @click="selected = 'Female'; $refs.user_gender.value = 'female'; open = false" class="group cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-orange-500 hover:text-white">
            <span class="flex items-center">
                <i class="fas fa-female mr-2 text-orange-500 group-hover:text-white"></i> Female
            </span>
        </li>
    </ul>
    <input type="hidden" name="user_gender" x-ref="user_gender" :value="selected === 'Select Gender' ? '' : selected">
</div>